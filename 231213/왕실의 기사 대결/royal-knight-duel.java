...
public class Main {
    ...
    public static void main(String[] args) throws Exception {
        ...
        for(int q=0 ; q<Q ; q++){
            ...
            //0위, 1오, 2아래, 3왼
            boolean goKill = move(idx, direction);
    ...
    static boolean move(int idx, int dir){
    ...
    static void kill(int idx){
        for(int i=0 ; i<knight.size() ; i++){
            if(idx-1==i) continue;
            int r = knight.get(i)[0];
            if(r==-1) continue;
            int c = knight.get(i)[1];
            int h = knight.get(i)[2];
            int w = knight.get(i)[3];
            int k = knight.get(i)[4];
            int attack = knight.get(i)[5];
            ...
            knight.set(i, new int[]{r, c, h, w, k, attack});
            for(int ii=r ; ii<r+h ; ii++){
                for(int jj=c ; jj<c+w ; jj++){
                    if(k<=attack) {
                        knight.set(i, new int[]{-1});
                        zone[ii][jj]=0;
                    }        
                }
            }
